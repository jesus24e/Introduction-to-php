...
| [`bases_de_datos_php`](bases_de_datos_php) | Conexión a MySQL mediante MySQLi y operaciones básicas: crear una base y tabla, insertar, consultar, actualizar y eliminar registros. | `ejemplo_conexion.php`, `creacion_db.php`, `leer_db.php` |
...
| Herramienta | Uso |
| --- | --- |
| PHP 7.4 o superior | Ejecutar los ejercicios. Las funciones flecha usadas en callbacks requieren PHP 7.4+. |
...
| Extensiones `mysqli` y `ftp` | `mysqli` se requiere para base de datos y `ftp` solo para `conceptos_avanzados/ftp.php`. |
...
5. Para los ejemplos de base de datos, ejecuta primero `bases_de_datos_php/creacion_db.php`. Este crea la base `test_php`, la tabla `empleados` y un registro de muestra; después puedes probar la lectura, actualización y eliminación.
